...
const InquiryHistoryPage = ({ embedded = false }) => {
  const { user } = useAuth();
  ...
  const mainContent = (
    <>
        <header style={{ marginBottom: '40px' }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: embedded ? '1.8rem' : '2.5rem', margin: 0 }}>History</h1>
            ...
            >
              ...
              >
                ...
                <div style={{ padding: '30px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  {selectedInquiry.status === 'completed' && selectedInquiry.service && (
                    <div style={{ marginTop: '10px', padding: '25px', background: 'rgba(255,255,255,0.02)', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.05)' }}>
                      <h4 style={{ color: '#C9A84C', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '1.1rem' }}>
                        <HiCheckCircle /> Service Member Details
                      </h4>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        <DetailRow label="Member / Business" value={selectedInquiry.service.name || 'Premium Provider'} />
                        {selectedInquiry.service.type && <DetailRow label="Service Type" value={selectedInquiry.service.type.replace('_', ' ').toUpperCase()} />}
                        {selectedInquiry.service.email && <DetailRow label="Contact Email" value={selectedInquiry.service.email} />}
                        {selectedInquiry.service.phone && <DetailRow label="Phone Number" value={selectedInquiry.service.phone} />}
                        {selectedInquiry.service.instagram && <DetailRow label="Instagram" value={`@${selectedInquiry.service.instagram}`} />}
                      </div>
                    </div>
                  )}
